...
export default function Edit( { attributes, setAttributes } ) {
	...
	const taxonomyOptions = [
		{ value: '', label: __( '— Auto (use page context) —', 'codelag-blocks' ) },
		...( taxonomies || [] )
	...
	const previewTaxonomy =
		taxonomy ||
		( taxonomies || [] ).find( ( t ) => t.slug.startsWith( 'lagoon_' ) )?.slug ||
		'';
	...
	const terms = useSelect(
		( select ) => {
			...
			return select( coreStore ).getEntityRecords( 'taxonomy', previewTaxonomy, {
				per_page: 50,
			...
			<InspectorControls>
				<PanelBody title={ __( 'Source', 'codelag-blocks' ) } initialOpen={ true }>
					<SelectControl
						...
						help={ __( 'Leave on Auto to follow the current term-index page (e.g. /snippets/language/).', 'codelag-blocks' ) }
						value={ taxonomy }
			...
			<div { ...blockProps }>
				{ ! previewTaxonomy && (
					<p className="codelag-term-index__hint">
						{ __( 'No taxonomy selected and none detected.', 'codelag-blocks' ) }
					</p>
				) }
				...
				{ previewTaxonomy && terms && terms.length > 0 && (
					<ul className="codelag-term-index__list">
						{ terms.map( ( term ) => (
							<li key={ term.id } className="codelag-term-index__item">
								<span className="codelag-term-index__name">{ term.name }</span>
								{ showCount && (
