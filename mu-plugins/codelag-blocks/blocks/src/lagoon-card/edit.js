...
export default function Edit( { attributes, setAttributes } ) {
	...
	const blockProps = useBlockProps( { className: 'codelag-card codelag-card--editor-preview' } );

	return (
		<>
			<InspectorControls>
				<PanelBody title={ __( 'Card', 'codelag-blocks' ) } initialOpen={ true }>
					<RangeControl
						label={ __( 'Preview lines', 'codelag-blocks' ) }
						value={ previewLines }
						onChange={ ( value ) => setAttributes( { previewLines: value } ) }
						min={ 2 }
						max={ 12 }
					/>
					<RangeControl
						...
						value={ maxTagChips }
						onChange={ ( value ) => setAttributes( { maxTagChips: value } ) }
						min={ 0 }
						max={ 10 }
					/>
					<ToggleControl
					...
					<ToggleControl
						...
						checked={ showFork }
						onChange={ ( value ) => setAttributes( { showFork: value } ) }
					/>
				</PanelBody>
			</InspectorControls>
			...
			<article { ...blockProps }>
				<header className="codelag-card__head">
					...
					<div className="codelag-card__ident">
						<span className="codelag-card__title">{ __( 'Lagoon title', 'codelag-blocks' ) }</span>
						<span className="codelag-card__meta">
							{ __( 'by @author · recently', 'codelag-blocks' ) }
						</span>
					</div>
					<span className="codelag-chip codelag-chip--lang">LANG</span>
				</header>
				...
				<ul className="codelag-card__tags" aria-hidden="true">
					{ Array.from( { length: Math.min( maxTagChips, 3 ) } ).map( ( _, i ) => (
						<li key={ i } className="codelag-chip codelag-chip--tag">#tag</li>
					) ) }
				</ul>
