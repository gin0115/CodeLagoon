...
export default function Edit( { attributes, setAttributes } ) {
	...
	return (
		<>
			<InspectorControls>
				<PanelBody
					title={ __( 'Filters', 'codelag-blocks' ) }
					initialOpen={ true }
				>
					<ToggleControl
					...
					<ToggleControl
						...
						checked={ showPurpose }
						onChange={ ( value ) =>
							setAttributes( { showPurpose: value } )
						}
					/>
					<ToggleControl
						label={ __(
							'Show tag multi-select',
							'codelag-blocks'
						) }
						checked={ showTags }
						onChange={ ( value ) =>
					...
					<ToggleControl
						label={ __(
							'Auto-hide current taxonomy',
							'codelag-blocks'
						) }
						help={ __(
							'On a taxonomy archive, hide the filter that matches the current term.',
							'codelag-blocks'
						) }
						checked={ autoHideCurrentTerm }
						onChange={ ( value ) =>
							setAttributes( { autoHideCurrentTerm: value } )
						}
					/>
				</PanelBody>
				<PanelBody
					title={ __( 'Call to action', 'codelag-blocks' ) }
					initialOpen={ false }
				>
					<ToggleControl
						label={ __(
						...
						checked={ showNewCta }
						onChange={ ( value ) =>
							setAttributes( { showNewCta: value } )
						}
					/>
					<TextControl
						label={ __( 'New entry URL', 'codelag-blocks' ) }
						help={ __(
							'Leave blank to use wp-admin new-post URL.',
							'codelag-blocks'
						) }
						value={ newEntryUrl }
						onChange={ ( value ) =>
							setAttributes( { newEntryUrl: value } )
						}
					/>
				</PanelBody>
			</InspectorControls>
			...
			<form { ...blockProps } onSubmit={ ( e ) => e.preventDefault() }>
				{ showSearch && (
					<span className="codelag-filters__slot codelag-filters__slot--search">
						...
						<span className="codelag-filters__placeholder">
							{ __(
								'Keyword, hash, or author…',
								'codelag-blocks'
							) }
						</span>
					</span>
				...
				{ showLanguage && (
					<span className="codelag-filters__slot">
						<span className="codelag-filters__label">
							{ __( 'Language', 'codelag-blocks' ) }
						</span>
					</span>
				) }
				{ showDate && (
