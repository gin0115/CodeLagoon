...
export default function Edit( { attributes, setAttributes } ) {
	const { taxonomy, count, orderBy, parentLabel, parentUrl, showCount } =
		attributes;
	...
	const blockProps = useBlockProps( {
		className:
			'wp-block-navigation-item wp-block-navigation-submenu codelag-term-submenu',
	} );
	...
	const resolvedLabel =
		...
		( taxonomy &&
			taxonomies &&
			( taxonomies.find( ( t ) => t.slug === taxonomy )?.name ||
				taxonomy ) ) ||
	...
	return (
		<>
			<InspectorControls>
				...
				<PanelBody
					title={ __( 'Parent link', 'codelag-blocks' ) }
					initialOpen={ true }
				>
					<TextControl
						label={ __( 'Label', 'codelag-blocks' ) }
						help={ __(
							'Shown as the submenu trigger. Defaults to the taxonomy name.',
							'codelag-blocks'
						) }
						value={ parentLabel }
						onChange={ ( value ) =>
							setAttributes( { parentLabel: value } )
						}
					/>
					<TextControl
						label={ __( 'URL', 'codelag-blocks' ) }
						help={ __(
							'Where the parent label links to. Leave blank to make the label a non-link toggle.',
							'codelag-blocks'
						) }
						value={ parentUrl }
						onChange={ ( value ) =>
							setAttributes( { parentUrl: value } )
						}
					/>
				</PanelBody>
			</InspectorControls>
				...
				<ul className="wp-block-navigation__submenu-container codelag-term-submenu__list">
					{ ! taxonomy && (
						<li className="codelag-term-submenu__hint">
							{ __(
								'Pick a taxonomy in the sidebar.',
								'codelag-blocks'
							) }
						</li>
					) }
						...
						terms.map( ( term ) => (
							...
							>
								<span className="wp-block-navigation-item__content">
									{ term.name }
									{ showCount && (
										<span className="codelag-term-submenu__count">
											{ ' ' }
											({ term.count || 0 })
										</span>
									) }
