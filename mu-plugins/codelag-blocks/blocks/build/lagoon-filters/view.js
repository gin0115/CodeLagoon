...
(function () {
  ...
  function initSelect2(form) {
    ...
    form.querySelectorAll('select[multiple]').forEach(function (select) {
      ...
      const labelText = label ? (label.querySelector('.codelag-filters__label') || {}).textContent : '';
      $(select).select2({
  ...
  function submit(form, grid) {
    ...
    const url = restBase + 'codelag/v1/lagoons' + (fetchQs ? '?' + fetchQs : '');
  ...
  function syncFormFromParams(form, params) {
    ...
    ['filter_language', 'filter_purpose', 'filter_tag'].forEach(function (name) {
      ...
      const values = params.getAll(name + '[]').concat(params.getAll(name));
  ...
  function cardHtml(item) {
    const title = item.title ? escape(item.title) : escape(item.slug || '');
    const permalink = escape(item.link || '#');
    const author = item.author && item.author.username ? escape(item.author.username) : '';
    const authorUrl = item.author && item.author.archive_url ? escape(item.author.archive_url) : '#';
    const authorDisp = author ? '<a class="codelag-card__author" href="' + authorUrl + '">@' + author + '</a>' : '';
    const ago = relativeTime(item.date);
    const langName = item.languages && item.languages[0] ? escape(item.languages[0].name) : '';
    const firstFile = item.files && item.files[0] ? item.files[0] : null;
    const preview = firstFile ? escape((firstFile.content || '').split('\n').slice(0, 6).join('\n')) : '';
    const tags = (item.tags || []).slice(0, 4).map(function (t) {
