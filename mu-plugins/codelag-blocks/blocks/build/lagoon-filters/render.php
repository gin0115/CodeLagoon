...
( static function ( array $attributes, string $content, WP_Block $block ): void {
	...
	// Form action — the current archive URL if we're on one, otherwise the
	// CPT archive. This keeps taxonomy archives "sticky" when submitting
	// without JS.
	...
	// phpcs:enable WordPress.Security.NonceVerification.Recommended
	$current_languages = $collect_multi( 'language' );
	$current_purposes  = $collect_multi( 'purpose' );
	$current_tags      = $collect_multi( 'tag' );
