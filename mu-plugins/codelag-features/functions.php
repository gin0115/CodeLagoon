 ...
 */
function codelag_features_get_metadata( ?string $property = null ) {
	...
	if ( null === $plugin_data ) {
		...
		$plugin_data = get_plugin_data( __DIR__ . '/codelag-features.php', true, false );
	}
 ...
 * @return  string
 */
function codelag_features_get_slug(): string {
	return 'codelag-features';
}
 ...
 */
function codelag_features_get_asset_meta( string $asset_path, ?array $extra_dependencies = null ): ?array {
	$asset_path = str_starts_with( $asset_path, constant( 'CODELAG_FEATURES_DIR_PATH' ) ) ? $asset_path : constant( 'CODELAG_FEATURES_DIR_PATH' ) . $asset_path;
	...
	if ( '' === $asset_meta['version'] ) {
		$asset_meta['version'] = codelag_features_get_metadata( 'Version' );
	}
